...
import br.jpe.dallahits.gen.entidade.ClienteEntidade;
 ...
 * @author Joaov
 */
public class ClienteGrid extends AbstractGrid {
    ...
    protected JSONArray getTitulos() {
        return new ClienteEntidade().getTitulos();
    }
    ...
    protected JSONArray getColunas() {
        return new ClienteEntidade().getColunas();
    }
    ...
    public List<? extends AbstractBean> getDados() throws DallaHitsException {
        try (Conexao conn = ConnFactory.criaConexao()) {
            return new ClienteDAO(conn).busca();
